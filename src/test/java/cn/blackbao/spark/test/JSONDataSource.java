...
private static final Logger LOGGER = LoggerFactory.getLogger(JSONDataSource.class);
	...
	public static void main(String[] args) {
		SparkSession sparkSession = SparkSession.builder().appName("jsonDataSource").getOrCreate();
//		JavaSparkContext sc = JavaSparkContext.fromSparkContext(sparkSession.sparkContext());
//		JavaRDD<String> rdd = sc.textFile("word.json");
//		Dataset<Row> json = sparkSession.read().json(rdd);
...
//		StructType structType = DataTypes.createStructType(fields);
		...
		result.foreach(new VoidFunction<Row>() {
			private static final long serialVersionUID = 1L;
			@Override
			public void call(Row row) throws Exception {
				String sql = "insert into result (`name`,`age`,`clazz`,`score`) values ('"+
							row.getString(0)+"','"+
							row.getInt(1)+"','"+
							row.getString(2)+"','"+
							row.getInt(3)	+"')";
				...
				try {
					connection = DriverManager.getConnection("jdbc:mysql://192.168.123.220:3306/spark?useSSL=false&characterEncoding=UTF8","root","1234");
