...
import cn.blackbao.spark.entity.SessionDetail;
...
import cn.blackbao.spark.utills.DateUtils;
import cn.blackbao.spark.utills.MockData;
import cn.blackbao.spark.utills.NumberUtils;
import cn.blackbao.spark.utills.ParamUtils;
import cn.blackbao.spark.utills.StringUtils;
import cn.blackbao.spark.utills.ValidUtils;
...
//	@Autowired
//	private IAdBlacklistDao adBlacklistDao;
//	@Autowired
//	private IAdClickTrendDao adClickTrendDao;
//	@Autowired
//	private IAdProvinceTop3Dao adProvinceTop3Dao;
//	@Autowired
//	private IAdStatDao adStatDao;
//	@Autowired
//	private IAdUserClickCountDao adUserClickCountDao;
//	@Autowired
//	private IAreaTop3ProductDao areaTop3ProductDao;
//	@Autowired
//	private IPageSplitConvertRateDao pageSplitConvertRateDao;
//	@Autowired
//	private ISessionAggrStatDao sessionAggrStatDao;
//	@Autowired
//	private ISessionDetailDao sessionDetailDao;
//	@Autowired
//	private ISessionRandomExtractDao sessionRandomExtractDao;
//	@Autowired
//	private ITop10CategoryDao top10CategoryDao;
//	@Autowired
//	private ITop10SessionDao top10SessionDao;
//	
...
//		sc.setCheckpointDir(".");
//		sparkSession = SparkSession.builder().appName("assasaas").getOrCreate();
//		try {
...
//		long taskId = ParamUtils.getTaskIdFromArgs(null, Constants.SPARK_LOCAL_TASKID_SESSION);
//		SessionAggrStatAccumulator accumulator = new SessionAggrStatAccumulator();
//		sc.sc().register(accumulator,"count");
//		Task task = taskDao.findById(taskId);
//		System.err.println(task);
		
//		JSONObject taskObject = JSON.parseObject("{\"startDate\":\"2018-01-14\",\"endDate\":\"2018-05-14\"}");
//		JavaRDD<Row> actionRDD = getActionRDDByDateRange(sparkSession, taskObject);
//		JavaPairRDD<String, String> session2UserInfo = aggregateBySession(sparkSession, actionRDD);
//		
//		long count = session2UserInfo.count();
//		System.err.println(count);
//		JavaPairRDD<String, String> filterSessionAndAggrStat = filterSessionAndAggrStat(session2UserInfo, taskObject);
//		JavaPairRDD<String, String> filteredRDD = filterSession(session2UserInfo, taskObject);
//		long count2 = filterSessionAndAggrStat.count();
...
//		sc.close();
	...
	public static void main(String[] args) {
		...
		conf.registerKryoClasses(new Class[]{Task.class});
		try {
			MockData.mock(sc, sparkSession);
		} catch (AnalysisException e) {
			e.printStackTrace();
...
//		System.err.println(count);
		JavaPairRDD<String, String> filterSessionAndAggrStat = filterSessionAndAggrStat(session2UserInfo, taskObject);
//		JavaPairRDD<String, String> filteredRDD = filterSession(session2UserInfo, taskObject);
//		long count2 = filteredRDD.count();
		long count2 = filterSessionAndAggrStat.count();
		System.err.println(count2);
	
	}
	
	private static JavaRDD<Row> getActionRDDByDateRange(SparkSession sparkSession,JSONObject taskObject){
		String startDate = ParamUtils.getParam(taskObject, "startDate");
		String endDate = ParamUtils.getParam(taskObject, "endDate");
	...
	private static JavaPairRDD<String, String> aggregateBySession(SparkSession sparkSession,JavaRDD<Row> actionRDD){
		...
		JavaPairRDD<String, Iterable<Row>> groupByKey = mapToPair.groupByKey();
		
		JavaPairRDD<Long, String> userPartAggr = groupByKey.mapToPair(new PairFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
			private static final long serialVersionUID = 1L;
			@Override
			public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
				String session = tuple._1;
				...
				while (iterator.hasNext()) {
					...
					if(clickCategoryId!=null) {
						if(!clickCategoryIdBuffer.toString().contains(String.valueOf(clickCategoryId))) {
							clickCategoryIdBuffer.append(String.valueOf(clickCategoryId)+",");
						}
					}
				}
				
				String searchKeyword = StringUtils.trimComma(searchKeywordBuffer.toString());
				String clickCategoryId = StringUtils.trimComma(clickCategoryIdBuffer.toString());
				result.put("session", session);
				result.put("searchKeyword", searchKeyword);
				result.put("clickCategoryId", clickCategoryId);
				return new Tuple2<Long, String>(userId, JSON.toJSONString(result));
			}
		});
		
		//查询所有用户信息
		String sql = "select * from user_info";
		JavaRDD<Row> userRDD = sparkSession.sql(sql).javaRDD();
		JavaPairRDD<Long, Row> userInfo = userRDD.mapToPair(new PairFunction<Row, Long, Row>() {
			private static final long serialVersionUID = 1L;
			@Override
		...
		//通过用户信息，jion用户和session
		JavaPairRDD<String, String> session2UserInfo = userInfo.join(userPartAggr).mapToPair(new PairFunction<Tuple2<Long,Tuple2<Row,String>>, String, String>() {
			private static final long serialVersionUID = 1L;
			@Override
			public Tuple2<String, String> call(Tuple2<Long, Tuple2<Row, String>> tuple) throws Exception {
				String sessionAggrInfo = tuple._2._2;
				Row userInfo = tuple._2._1;
				JSONObject sessionJSON = JSON.parseObject(sessionAggrInfo);
				String session = sessionJSON.getString("session");
				...
				return new Tuple2<String, String>(session, jsonResult);
			}
		});
		return session2UserInfo;
	}

	private static JavaPairRDD<String, String> filterSession(JavaPairRDD<String, String> rdd , JSONObject taskParam){
...
//		
	 ...
	 * @param sessionid2AggrInfoRDD  
	 */
	private void randomExtractSession(
			JavaSparkContext sc,
			final long taskid,
			JavaPairRDD<String, String> sessionid2AggrInfoRDD,
			JavaPairRDD<String, Row> sessionid2actionRDD) { 
...
//								ISessionRandomExtractDao sessionRandomExtractDao2 = getSessionRandomExtractDao();
//								sessionRandomExtractDao2.insert(sessionRandomExtract);
...
//						ISessionDetailDao sessionDetailDao2 = getSessionDetailDao();
//						sessionDetailDao2.insertBatch(sessionDetails);;
					}
				});
	}
	...
	private void calculateAndPersistAggrStat(String value, long taskid) {
		...
		// 调用对应的DAO插入统计结果
//		ISessionAggrStatDao sessionAggrStatDao2 = getSessionAggrStatDao();
//		sessionAggrStatDao2.insert(sessionAggrStat);
	}
	 ...
	 * @param sessionid2actionRDD
	 */
	private  List<Tuple2<CategorySortKey, String>> getTop10Category(  
			long taskid,  
...
//		ITop10CategoryDao top10CategoryDao2 = getTop10CategoryDao();
...
//			top10CategoryDao2.insert(category);  
		}
...
//								ITop10SessionDao top10SessionDao = getTop10SessionDao();
...
//								top10SessionDao.insert(top10Session);  
		...
		sessionDetailRDD.foreach(new VoidFunction<Tuple2<String,Tuple2<String,Row>>>() {  
			...
			public void call(Tuple2<String, Tuple2<String, Row>> tuple) throws Exception {
				...
				sessionDetail.setPayCategoryIds(row.getString(10)); 
				sessionDetail.setPayProductIds(row.getString(11));  
//				ISessionDetailDao sessionDetailDao = getSessionDetailDao();
//				ISessionDetailDao sessionDetailDAO = DAOFactory.getSessionDetailDAO();
//				sessionDetailDao.insert(sessionDetail);  
			}
		});
...
//	public IAdBlacklistDao getAdBlacklistDao() {
//		return adBlacklistDao;
//	}
//
//	public IAdClickTrendDao getAdClickTrendDao() {
//		return adClickTrendDao;
//	}
//
//	public IAdProvinceTop3Dao getAdProvinceTop3Dao() {
//		return adProvinceTop3Dao;
//	}
//
//	public IAdStatDao getAdStatDao() {
//		return adStatDao;
//	}
//
//	public IAdUserClickCountDao getAdUserClickCountDao() {
//		return adUserClickCountDao;
//	}
//
//	public IAreaTop3ProductDao getAreaTop3ProductDao() {
//		return areaTop3ProductDao;
//	}
//
//	public IPageSplitConvertRateDao getPageSplitConvertRateDao() {
//		return pageSplitConvertRateDao;
//	}
//
//	public ISessionAggrStatDao getSessionAggrStatDao() {
//		return sessionAggrStatDao;
//	}
//
//	public ISessionDetailDao getSessionDetailDao() {
//		return sessionDetailDao;
//	}
//
//	public ISessionRandomExtractDao getSessionRandomExtractDao() {
//		return sessionRandomExtractDao;
//	}
//
//	public ITop10CategoryDao getTop10CategoryDao() {
//		return top10CategoryDao;
//	}
//
//	public ITop10SessionDao getTop10SessionDao() {
//		return top10SessionDao;
//	}
